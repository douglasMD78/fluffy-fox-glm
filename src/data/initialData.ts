...
export const PDF_LUIZA_DATA = (() => {
    ...
    const specialPages = originalData.filter(p => 
        p.type === TEMPLATES.COVER || 
        p.type === TEMPLATES.INTRO || 
        p.type === TEMPLATES.LEGEND
    );

    // Manter apenas a primeira página de sumário, as outras serão geradas dinamicamente
    const initialTocPage = originalData.find(p => p.type === TEMPLATES.TOC && (p as TocPageData).tocPageNumber === 1);
    ...
    USER_CATEGORIZED_RECIPES.forEach(itemTitle => {
        // Se for um título de categoria, adiciona uma página de seção
        if (itemTitle.endsWith("S") && itemTitle.length > 5) { // Heurística simples para identificar categorias
            const sectionTitle = itemTitle.replace(/S$/, '').trim(); // Remove o 'S' final para o título da seção
            newRecipeOrder.push({
                id: `p_section_${sectionTitle.toLowerCase().replace(/ /g, '_')}_${Date.now()}`,
                type: TEMPLATES.SECTION,
                title: itemTitle, // Usar o título completo da categoria
                subtitle: "" // Subtítulo vazio por padrão
            });
        } else {
