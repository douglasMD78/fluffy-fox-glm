...
export const IntroEditor: React.FC<IntroEditorProps> = ({ activePage, updatePage, onAiRequest }) => {
    return (
        <div className="space-y-4">
            ...
            <textarea placeholder="Texto" className="w-full bg-white border border-gray-200 p-4 rounded-xl text-xs leading-loose text-navy focus:border-accent outline-none" rows={15} value={activePage.text} onChange={e => updatePage({text: e.target.value})} />
        </div>
    );
};