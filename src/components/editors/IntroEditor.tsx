...
export const IntroEditor: React.FC<IntroEditorProps> = ({ data, onUpdate, onAiRequest }) => {
  return (
    <div className="space-y-4">
      ...
      <button 
        onClick={onAiRequest} 
        className="w-full py-2 bg-rose-50 border border-rose-100 text-rose-500 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-rose-100 transition-all"
      >
        <Sparkles size={12}/> ✨ Escrever Intro com IA
      </button>
      <MarkdownTextarea 
        placeholder="Texto" 
        rows={15} 
