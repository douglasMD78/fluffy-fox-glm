...
import { RecipeTip } from '@/components/common/RecipeTip';
import { RecipeStorage } from '@/components/common/RecipeStorage';
...
export const InfoFooter: React.FC<InfoFooterProps> = ({ data, compact = false, hideNutrition = false }) => {
    const showTipInFooter = data.tipPlacement === 'footer' && data.tips;
    const showStorageInFooter = data.storagePlacement === 'footer' && data.storage;

    if (!data.videoLink && !showTipInFooter && !showStorageInFooter && hideNutrition) {
        return null; // Don't render footer if nothing is visible
    }

    return (
        <div className={`mt-auto ${compact ? 'pt-2 space-y-2' : 'pt-3 space-y-2'}`}>
            {data.videoLink && data.videoDisplayStyle === 'button' && (
            ...
            <div className={`grid ${compact ? 'grid-cols-1 gap-1.5' : 'grid-cols-1 gap-2'}`}>
                {showTipInFooter && <RecipeTip tip={data.tips} compact={compact} />}
                {showStorageInFooter && <RecipeStorage storage={data.storage} compact={compact} />}
            </div>
